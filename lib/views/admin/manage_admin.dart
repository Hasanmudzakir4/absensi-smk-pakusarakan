import 'package:absensi_smk_pakusarakan/controllers/admin_controller.dart';
...
class _ManageAdminState extends State<ManageAdmin> {
  ...
  int get totalPages => (filteredAdmin.length / rowsPerPage).ceil();

  List<Map<String, dynamic>> get paginatedAdmin {
    final startIndex = currentPage * rowsPerPage;
    final endIndex = startIndex + rowsPerPage;
    if (startIndex >= filteredAdmin.length) {
      return [];
    }
    return filteredAdmin.sublist(
      startIndex,
      endIndex > filteredAdmin.length ? filteredAdmin.length : endIndex,
    );
  }

  Future<void> _loadAdmin() async {
    final result = await _adminController.getAllAdmin();
    if (!mounted) return;
  ...
  void _applyFilters() {
    List<Map<String, dynamic>> temp = List.from(admin);
  ...
  Future<void> exportToExcel(List<Map<String, dynamic>> data) async {
    ...
    html.AnchorElement(href: url)
      ..setAttribute('download', 'data_admin.xlsx')
      ..click();
  ...
  void _showEditAdminDialog(Map<String, dynamic> admin) {
    ...
    final nipController = TextEditingController(text: admin['idNumber'] ?? '');
    final phoneController = TextEditingController(text: admin['phone'] ?? '');

    // Pisahkan tempat & tanggal lahir
    final String rawBirth = admin['dateOfBirth'] ?? '';
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    ...
                    await _loadAdmin(); // Refresh data tabel
                  } catch (e) {
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                                            ...
                                            child: Scrollbar(
                                              ...
                                              child: SingleChildScrollView(
                                                ...
                                                child: DataTable(
                                                  ...
                                                  rows: List.generate(paginatedAdmin.length, (
                                                    index,
                                                  ) {
                                                    final admin =
                                                        paginatedAdmin[index];
                                                    return DataRow(
                                                      cells: [
                                                        DataCell(
                                                          Text(
